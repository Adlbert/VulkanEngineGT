...
void main() {
    vec4 ambientC = lightUBO.data.col_ambient;
    ...
    float s = 2.5f;
    vec4 n = vec4(fragNormal.x,fragNormal.y,fragNormal.z, 0.0f);
    ...
    vec4 diffuse = max(dot(n, L), 0.0f) * diffuseC;
    vec4 specular = pow(max(dot(n, H), 0.0f), s) * specularC;
