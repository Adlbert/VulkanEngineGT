...
namespace gjk {
	...
	bool sat_faces_test(Face& face1, Face& face2, vec3& dir) {
		...
		if (sat_axis_test(face2, face1, dir)) return true;
		return false;
	}
	...
	bool sat_faces_test(Polytope& obj1, Polytope& obj2, vec3& dir) {
		...
		for (auto& face : obj2.m_faces) {
			dir = face.get_face_normal();
			if (sat_axis_test(obj2, obj1, dir)) return true;
		}
	...
	bool sat(Face& face1, Face& face2, vec3& dir) {
		...
		if (sat_faces_test(face1, face2, sat_dir1)) return false;
		if (sat_edges_test(face1, face2, sat_dir2)) return false;
		return true;
	}
